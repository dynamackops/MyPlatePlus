...
export default function App({ supabaseConfig }: { supabaseConfig: SupabasePublicConfig }) {
  ...
  return (
    <div className={`app theme-${profile.theme} ${preferences.reducedMotion ? 'pref-reduced-motion' : ''} ${preferences.largeText ? 'pref-large-text' : ''} ${preferences.highContrast ? 'pref-high-contrast' : ''} ${preferences.decorativeVisuals ? '' : 'pref-no-decor'} ${preferences.compactCards ? 'pref-compact' : ''}`}>
      ...
      {modal === 'focus' && <FocusDisplay items={items} capacity={capacity} used={used} percent={percent} onAdd={() => setModal('add')} onEdit={(item) => { setSelectedItem(item); setModal('edit') }} onClose={() => setModal('none')} />}
    </div>
  )
}
...
function PlateView({ name, items, capacity, used, percent, fullness, preferences, onCheckin, onBrain, onAdd, onRoom, onFocus, onEdit, onPass }: {
  ...
  onCheckin: () => void; onBrain: () => void; onAdd: () => void; onRoom: () => void; onFocus: () => void; onEdit: (item: PlateItem) => void; onPass: (item: PlateItem) => void
}) {
...
function FocusDisplay({ items, capacity, used, percent, onAdd, onEdit, onClose }: { items: PlateItem[]; capacity: number; used: number; percent: number; onAdd: () => void; onEdit: (item: PlateItem) => void; onClose: () => void }) {
  const active = items.filter((item) => item.status === 'active')
  ...
  return <div className="focus-backdrop" role="dialog" aria-modal="true" aria-label="Focus display"><header className="focus-header"><a className="brand" href="#top"><span className="brand-mark">+</span><span>MyPlate<b>+</b></span></a><div><b>{new Intl.DateTimeFormat('en-US', { weekday: 'long', month: 'long', day: 'numeric' }).format(new Date())}</b><small>A gentle view of today</small></div><div><button className="secondary-button" onClick={fullscreen}><Maximize2 size={17} /> Fullscreen</button><button className="icon-button" onClick={onClose} aria-label="Close focus display"><X /></button></div></header><main className="focus-layout"><section className="focus-plate-card"><div className="focus-capacity"><strong>{percent}%</strong><span><b>{used > capacity ? 'OVERFLOWING' : percent > 80 ? 'NEARLY FULL' : 'OPEN'}</b><small>{used} of {capacity} points · capacity is information</small></span></div><div className="focus-plate"><div>{active.map((item, index) => <button key={item.id} className={`plate-object object-${index % 6}`} style={{ '--object-size': `${Math.max(74, item.points * 3)}px`, '--object-color': categoryColor[item.category] } as React.CSSProperties} onClick={() => onEdit(item)}><span>{item.icon}</span><strong>{item.title}</strong><small>{item.points} pts</small></button>)}{active.length === 0 && <div className="empty-plate"><Sparkles /><b>Your plate is clear.</b><span>Add a commitment when you are ready.</span><button className="primary-button" onClick={onAdd}><Plus size={17} /> Add your first item</button></div>}</div></div></section><aside className="focus-sidebar"><article><p className="eyebrow">A GOOD PLACE TO START</p><h2>Three gentle next moves</h2>{nextMoves.length ? nextMoves.map((item, index) => <button key={item.id} onClick={() => onEdit(item)}><span>{index + 1}</span><div><b>{item.title}</b><small>{item.points} points{item.steps?.[0] ? ` · ${item.steps[0]}` : ''}</small></div><ChevronRight /></button>) : <p>Your active list is clear. Take a breath.</p>}</article><article><p className="eyebrow">UPCOMING</p><h2>{upcoming ? upcoming.title : 'Nothing urgent is due.'}</h2><p>{upcoming ? `Due ${new Date(`${upcoming.due}T12:00:00`).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}` : 'There’s no upcoming date asking for your attention.'}</p></article><blockquote>“Capacity is information,<br />not a measure of worth.”</blockquote></aside></main></div>
}
