...
export function AuthGate({ supabase, onDemo }: { supabase: SupabaseClient | null; onDemo: () => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setState('loading')
    const { error } = await requestMagicLink(supabase, email)
    ...
    <main className="auth-shell">
      ...
      <section className="auth-panel">
        <div className="auth-card">
          ...
          {!supabase && <div className="setup-note"><ShieldCheck size={18} /><span><strong>Hackathon preview</strong> Account sign-in activates when the MyPlate+ Supabase project is connected. You can explore everything below.</span></div>}
          {state === 'sent' ? (
            ...
            <form onSubmit={submit}>
              ...
              <button className="primary-button full" disabled={state === 'loading' || !supabase}>
