...
using (sender_id = (select auth.uid()) or recipient_id = (select auth.uid()) or (recipient_id is null and private.is_circle_member(circle_id)));
...
with check (
  sender_id = (select auth.uid()) and private.is_circle_member(circle_id)
  and (recipient_id is null or exists (
    select 1 from public.circle_members recipient
    where recipient.circle_id = pass_requests.circle_id and recipient.user_id = pass_requests.recipient_id
  ))
);
...
begin
  ...
  update public.pass_requests set status = decision, recipient_id = coalesce(target.recipient_id, (select auth.uid())), responded_at = now() where id = request_id;
  insert into public.sharing_audit (actor_id, circle_id, action, resource_id) values ((select auth.uid()), target.circle_id, 'request_updated', request_id);
  return request_id;
end;
$$;
